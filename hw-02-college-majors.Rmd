...
- *Because, unlike the mean value, the median doesn't change as much when a small number of extreme outliers are presented on one side of the distribution of the data. The mean would average them out with the rest, and would become quite skewed, while the median would only change as much as if the extra observations were added at the top side of the normal range of observations.*
...
- *I chose the higher binwidth value of $5000 because, when viewed at a binwidth if $1000, there are gaps in the data in between many large peaks (especially notable around multiples of $10,000), while the overall distribution is more easily visible at a binwidth of $5000.*

We can also calculate summary statistics for this distribution using the `summarise` function:

```{r eval =TRUE}
recent_grads_summary <- college_recent_grads %>%
...
- *The distribution of the graph is right-skewed, a fact which is exacerbated by the existence of a single outlier far to the right side of the graph. Because of the skewed-ness of this graph, using the median and quartiles would probably be a better idea than using the mean and standard deviation.*

- *The histogram is right-skewed, with a median of $`r format(recent_grads_summary$med, scientific = FALSE)` and an IQR of $`r format(recent_grads_summary$q3 - recent_grads_summary$q1, scientific = FALSE)`. It also has an outlier at $`r format(filter(college_recent_grads, median >= 100000)$median, scientific = FALSE)`, very far away from the next-highest value, significantly adding to the skew of the distribution.*
...
```{r eval=TRUE}
average_medians_ordered <- college_recent_grads %>%
  group_by(major_category) %>%
  summarise(med = median(median)) %>%
  arrange(desc(med))
average_medians_ordered
```

- *The major category with the highest average median income is `r slice(average_medians_ordered, 1)$major_category`, *
...
```{r, eval=TRUE}
majors_sort_unpopular <- college_recent_grads %>%
  count(major_category) %>%
  arrange(n)
majors_sort_unpopular
```

- *The least popular major category is `r slice(majors_sort_unpopular, 1)$major_category`, with `r slice(majors_sort_unpopular, 1)$n` observation(s).*
...
```{r}
college_recent_grads %>%
  ggplot(aes(x = median, y = sharewomen, colour = major_type)) +
  geom_point() +
  labs(title = "College majors", subtitle = "by average income and proportion of women", x = "Average income", y = "Proportion of women in major", colour = "Major type")
```

- *In both STEM fields and non-STEM fields, there is a negative correlation between the proportion of women in the major and the average income. The correlation is more strongly negative for non-STEM fields than for STEM fields, but, on average, STEM fields have a lower proportion of women than non-STEM fields.*

## Further exploration

10. Ask a question of interest to you, and answer it using summary statistic(s) and/or visualization(s).

### *Expanding on the previous question,we ask, 'what is the relation between major category and proportion of women?'*

```{r}
college_recent_grads %>%
  filter(median < 100000) %>% #Removing the engineering outlier.
  group_by(major_category) %>% 
  ggplot(aes(x = median, y = sharewomen, colour = major_category)) +
  geom_point(size = 0.5, alpha = 0.3) +
  facet_wrap(~ major_category) +
  coord_flip() +
  labs(title = "College majors", subtitle = "by average income and proportion of women", x = "Average income", y = "Proportion of women in major", colour = "Major category")
```

- *The major groups with the highest proportion of women in them seem to be Education, Health, Humanities & Liberal Arts, Psychology & Social Works, and Social Sciences, with Psychology and Education seeming to be the highest. There do not seem to be any groups with both high median incomes and proportion of women, while the other 3 corners and the center have major groups that cluster around them.*
